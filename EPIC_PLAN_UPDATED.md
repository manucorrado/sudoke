...
Audit of the monorepo (`apps/mobile`, `apps/api`, `apps/admin`, `packages/sudoku-core`) against each epic's deliverables and exit criteria.

| Epic | Status | Approx. | Key evidence |
|------|--------|---------|--------------|
...
| **7** Practice & Archive | **In progress** | **~90%** | `/archive` + `/archive/upcoming` + `/archive/{id}` + `/archive/{id}/ghost-rank` + `/archive/{id}/my-result` endpoints, Play tab with three pills (Casual / Archive / Upcoming), archive detail view (`<ArchiveResultCard>`) surfacing the player's original result + read-only `view=historical` board, archive replay screen on the real `sudoku-core` engine with end-of-game ghost-rank banner, upcoming-calendar view (difficulty only — no puzzle leak). All exit criteria met (2026-06-30); only the optional, non-exit "persisted practice attempts" enhancement remains (intentionally counter to the by-design client-side replay). |
| **8** Streaks & Notifications | **In progress** | **~80%** | Backend streak tracking with auto-freeze (max 2, +1 per 7 completions, auto-consume on miss), idempotent per (user, day), hooked into `submit_attempt`. `/me/streak` + `/me/notifications/preferences` (GET/PATCH) endpoints **+ push device registry (`push_tokens`, `POST`/`DELETE /me/push-tokens`) + preference-aware Expo dispatch service (`services/notifications.py`, gated by `EXPO_PUSH_ENABLED`, off by default) + best-effort "final ranking ready" push fired from the finalize cron**. Profile tab shows live streak card with freeze chips and toggle-driven notification preferences. Outstanding: on-device token acquisition (native `expo-notifications`) + APNs/FCM credentials + the remaining three notification triggers (native QA/infra). |
...
- [x] Historical leaderboard read-only with the user's original result if played — new `GET /archive/{id}/my-result` returns the player's original ranked result (`played=false` for non-players, 404 only while the daily is still open), surfaced in a new archive detail view (`<ArchiveResultCard>`) alongside the `view=historical` board before replay (2026-06-30)
...
- [ ] All four notification types deliver on device (native QA required) — device-token registry (`push_tokens` + `POST`/`DELETE /me/push-tokens`) and a preference-aware dispatch service (`services/notifications.py`: Expo push, batched, gated by `EXPO_PUSH_ENABLED` — off by default) now exist, and the finalize cron fires a best-effort "final ranking ready" push to opted-in finalists (2026-06-30). Remaining: on-device Expo Notifications token acquisition (native module), APNs/FCM credentials, and the other three triggers (daily reminder, friend-challenged-you, beat-your-time)
